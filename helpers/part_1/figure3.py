    ...
    for year in years:
        for code in geo_list:
            for index in df.index:
                df.at[index, (year, code, "Headship Rate")] = df.at[index, (year, code, "Headship Population")] / df.at[
                    index, (year, code, "Total Population")]
...
def figure3_helper(geo_list: List[int]):
    df = pd.DataFrame(
        ...
        columns=pd.MultiIndex.from_product([years, geo_list, ["Headship Population", "Total Population"]])
    )
    flattened_dfs = {
    ...
    for code in geo_list:
        for i, index in enumerate(df.index):
            ...
            # for under 25
            if i == 0:
                df.at[index, (2006, code, "Headship Population")] = PHM_2006.at[code, "under 25 years"]
                df.at[index, (2006, code, "Total Population")] = \
                    flattened_dfs[2006].loc[code, [f'{i + 1}5 to {i + 1}9 years', f'{i + 2}0 to {i + 2}4 years']].sum()
                df.at[index, (2021, code, "Headship Population")] = flattened_dfs[2021].loc[code, f'15 to 24 years']
                df.at[index, (2021, code, "Total Population")] = \
                    flattened_dfs[2021].loc[code, [f'{i + 1}5 to {i + 1}9 years', f'{i + 2}0 to {i + 2}4 years']].sum()
            # for 75-84
            elif i == (len(df.index) - 2):
                df.at[index, (2006, code, "Headship Population")] = PHM_2006.at[code, '75 years+'] - PHM_2006_85.at[code]
                df.at[index, (2006, code, "Total Population")] = \
                    flattened_dfs[2006].loc[code, [f'{i + 1}5 to {i + 1}9 years', f'{i + 2}0 to {i + 2}4 years']].sum()
                df.at[index, (2021, code, "Headship Population")] = flattened_dfs[2021].loc[code, f'{i + 1}5 to {i + 2}4 years']
                df.at[index, (2021, code, "Total Population")] = \
                    flattened_dfs[2021].loc[code, [f'{i + 1}5 to {i + 1}9 years', f'{i + 2}0 to {i + 2}4 years']].sum()
            # for 85+
            elif i == (len(df.index) - 1):
                df.at[index, (2006, code, "Headship Population")] = PHM_2006_85.at[code]
                df.at[index, (2006, code, "Total Population")] = \
                    flattened_dfs[2006].loc[code, [f'{i + 1}5 to {i + 1}9 years', f'{i + 2}0 to {i + 2}4 years']].sum()
                # First instance is total population, second one is PHM
                df.at[index, (2021, code, "Headship Population")] = flattened_dfs[2021].loc[code, '85 years+'].iat[1]
                df.at[index, (2021, code, "Total Population")] = flattened_dfs[2021].loc[code, '85 years+'].iat[0]
            # for 25-74
            else:
                df.at[index, (2006, code, "Headship Population")] = PHM_2006.at[code, f'{i + 1}5 to {i + 2}4 years']
                df.at[index, (2006, code, "Total Population")] = \
                    flattened_dfs[2006].loc[code, [f'{i + 1}5 to {i + 1}9 years', f'{i + 2}0 to {i + 2}4 years']].sum()
                df.at[index, (2021, code, "Headship Population")] = flattened_dfs[2021].loc[code, f'{i + 1}5 to {i + 2}4 years']
                df.at[index, (2021, code, "Total Population")] = \
